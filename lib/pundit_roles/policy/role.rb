...
module Role
  ...
  private def build_associated_roles(role, associated_as)
    ...
    associated_as.each do |key, value|
      raise ArgumentError, "Associated as values must be either a Symbol, or an Array of Symbols, got #{value} "+
        "for #{key} instead" unless value.is_a? Symbol or value.is_a? Array
